...
%       zaburzone wartości funkcji kwadratowej 
    ...
    for index = 1:num_points
        ...
        length(reference_coefficients)
        length(computed_coefficients)
        % Konwersja na double dla wykresu
        interpolation_error_exact(index) = double(max(abs(vpa(computed_coefficients) - vpa(reference_coefficients))));
    end
    ...
    for index = 1:num_points
        ...
        % TODO:
        reference_coefficients = [vpa(0); vpa(0); vpa(a2); vpa(zeros(size_n - 3, 1))];
